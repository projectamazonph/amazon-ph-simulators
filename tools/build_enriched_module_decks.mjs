...
const moduleIllustrations = [
  'module-01-amazon-basics.svg',
  'module-02-what-is-ppc.svg',
  'module-03-money-math.svg',
  'module-04-campaign-structure.svg',
  'module-05-keywords-and-match-types.svg',
  'module-06-listing-readiness.svg',
  'module-07-campaign-setup.svg',
  'module-08-bids-and-budgets.svg',
  'module-09-search-terms-and-negatives.svg',
  'module-10-weekly-optimization.svg',
  'module-11-reporting-and-troubleshooting.svg',
  'module-12-va-workflow-and-capstone.svg'
];
...
const learningAids = {
  m0: { steps: ["Shopper search", "Product page", "Buy Box", "Order"], prompt: "What should you check before buying more clicks?", correct: "Check stock and product-page readiness first.", choices: ["Check stock and page readiness", "Raise the ad bid", "Add more targets"] },
  m1: { steps: ["Ad shown", "Shopper clicks", "Product page", "Order"], prompt: "What is the safest first PPC check?", correct: "Find the first weak step in the click-to-order path.", choices: ["Find the first weak funnel step", "Assume every click will buy", "Raise every bid"] },
  m2: { steps: ["Spend", "Clicks", "Sales", "Safe limit"], prompt: "What should happen before a money decision?", correct: "Calculate the result, then compare it with the product margin.", choices: ["Calculate, then compare with margin", "Judge sales without cost", "Change every number"] },
  m3: { steps: ["Campaign", "Ad group", "Target", "Review"], prompt: "When should work be split into separate campaigns?", correct: "Split work that needs different budgets, bids, or decisions.", choices: ["Split different campaign jobs", "Mix every target together", "Name campaigns by date only"] },
  m4: { steps: ["Keyword", "Shopper search", "Search report", "Decision"], prompt: "What should you read before blocking or promoting a term?", correct: "Read the shopper search and the evidence in the report.", choices: ["Read the report evidence", "Act after one click", "Treat keyword and search term as identical"] },
  m5: { steps: ["Ad click", "Listing", "Offer and trust", "Order"], prompt: "Clicks are coming but orders are weak. Check what first?", correct: "Check the product page before increasing ad pressure.", choices: ["Check the product page", "Raise the bid", "Add a larger budget"] },
  m6: { steps: ["Learning question", "Auto discovery", "Manual control", "Weekly review"], prompt: "What makes a new campaign safer to learn from?", correct: "Give each campaign one job and one learning question.", choices: ["One job and one question", "Many unclear campaigns", "Scale on launch day"] },
  m7: { steps: ["Bid", "Clicks", "Budget pace", "Review"], prompt: "What is a controlled optimization move?", correct: "Change one setting, set a safety limit, and choose a review date.", choices: ["Change one setting and review", "Change bid and budget together", "Keep editing every hour"] },
  m8: { steps: ["Search report", "Winner, waste, or maybe", "Action", "Check again"], prompt: "What makes a search term a safe negative?", correct: "It is clearly wrong for the product or keeps failing after enough evidence.", choices: ["Clearly wrong or proven waste", "No order after one click", "Any new search"] },
  m9: { steps: ["Baseline", "One change", "Observation window", "Change log"], prompt: "What should be recorded before an optimization edit?", correct: "Record the baseline and choose one variable to change.", choices: ["Record baseline and one variable", "React to one day", "Edit several settings"] },
  m10: { steps: ["Result", "Check product health", "Diagnose funnel", "Report next step"], prompt: "Sales fell but clicks stayed steady. What should you check first?", correct: "Check stock, price, reviews, and competing offers before changing ads.", choices: ["Check product health first", "Raise the bid", "Send a number-only report"] },
  m11: { steps: ["Task", "Permission", "SOP and evidence", "Approval or report"], prompt: "What should a VA do before a strategy change?", correct: "Check permission, follow the SOP, and prepare evidence for approval.", choices: ["Check permission and prepare evidence", "Change the strategy alone", "Skip the change log"] }
};

const escape = value => value.replace(/[&<>"']/g, character => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character]));

function visualAid(module, page, term, meaning, example) {
  const aid = learningAids[module.id];
  const mode = page % 3;
  if (mode === 0) {
    return `<section class="visual-panel diagram reveal d2" data-learning-aid="process-diagram" aria-label="Illustrated decision pathway"><span class="label">Word to know</span><div class="visual-term">${escape(term)}</div><span class="label">See the path</span><div class="step-flow">${aid.steps.map((step, index) => `<div class="step"><b>${index + 1}</b><span>${escape(step)}</span></div>`).join('<i aria-hidden="true">→</i>')}</div><p class="visual-caption">Follow the steps in order. Check the early step before changing the later step.</p></section>`;
  }
  if (mode === 1) {
    return `<section class="visual-panel table-panel reveal d2" data-learning-aid="comparison-table"><span class="label">Word to know</span><div class="visual-term">${escape(term)}</div><span class="label">Quick comparison</span><table><thead><tr><th>See</th><th>Ask</th></tr></thead><tbody><tr><td>${escape(term)}</td><td>What does it control?</td></tr><tr><td>Small example</td><td>What does the evidence show?</td></tr><tr><td>Next step</td><td>What is safe to check first?</td></tr></tbody></table><p class="visual-caption">Use this table to turn a new word into a safe decision.</p></section>`;
  }
  return `<section class="visual-panel illustration reveal d2" data-learning-aid="illustrated-decision"><span class="label">Word to know</span><div class="visual-term">${escape(term)}</div><span class="label">Decision compass</span><div class="compass"><div class="compass-core">${escape(term)}</div><div class="compass-point north">LOOK</div><div class="compass-point east">CHECK</div><div class="compass-point south">DECIDE</div><div class="compass-point west">LOG</div></div><p class="visual-caption">Look at the evidence, check the situation, make one safe decision, then log it.</p></section>`;
}

function specialAid(module, kind, term, meaning, example) {
  const aid = learningAids[module.id];
  if (kind === 'cover') {
    return `<section class="cover-aid reveal d2" data-learning-aid="lesson-map"><div class="cover-layout"><div><span class="label">Today’s learning map</span><div class="step-flow">${aid.steps.map((step, index) => `<div class="step"><b>${index + 1}</b><span>${escape(step)}</span></div>`).join('<i aria-hidden="true">→</i>')}</div><p class="visual-caption">${escape(meaning)}</p></div><figure class="cover-graphic"><img src="../../../assets/screenshots/ppc-coach/${moduleIllustrations[module.number]}" alt="${escape(module.title)} course illustration"><figcaption>Course visual</figcaption></figure></div></section>`;
  }
  if (kind === 'exercise') {
    return `<section class="exercise reveal d2" data-learning-aid="interactive-exercise"><span class="label">Try the decision</span><p class="exercise-question">${escape(aid.prompt)}</p><div class="choice-grid">${aid.choices.map((choice, index) => `<button class="choice-btn" type="button" data-correct="${index === 0}">${escape(choice)}</button>`).join('')}</div><p class="feedback" aria-live="polite">Choose an answer. Then compare it with the debrief below.</p><div class="debrief"><b>Debrief:</b> ${escape(aid.correct)}</div></section>`;
  }
  if (kind === 'guardrail') {
    return `<section class="safety-grid reveal d2" data-learning-aid="do-dont-comparison"><div class="safe"><span class="label">Do first</span><p>${escape(example)}</p></div><div class="risk"><span class="label">Do not do yet</span><p>${escape(meaning)}</p></div></section>`;
  }
  if (kind === 'evidence') {
    return `<section class="evidence-panel reveal d2" data-learning-aid="evidence-table"><span class="label">Use this note structure</span><table><thead><tr><th>What I saw</th><th>Safe action</th><th>Owner / review</th></tr></thead><tbody><tr><td>${escape(term)}</td><td>${escape(meaning)}</td><td>Write a name and check date</td></tr><tr><td>${escape(example)}</td><td>State one reason</td><td>Save the note</td></tr></tbody></table></section>`;
  }
  return `<section class="assessment-grid reveal d2" data-learning-aid="assessment-gate"><div><span class="label">Practice</span><b>${escape(module.practice)}</b><p>Pass at ${escape(module.score)}.</p></div><div><span class="label">Evidence</span><b>One decision note</b><p>Explain what you checked and why.</p></div><div><span class="label">Quiz</span><b>${escape(module.quiz)}</b><p>Take the quiz after your practice note.</p></div></section>`;
}

function slide(module, page, kind, eyebrow, title, term, meaning, example, action) {
  const isConcept = kind === 'concept';
  const aid = isConcept ? visualAid(module, page, term, meaning, example) : specialAid(module, kind, term, meaning, example);
  const explanation = isConcept ? `<section class="explain-panel reveal d3"><span class="label">Plain meaning</span><p class="body-copy">${escape(meaning)}</p><span class="label example-label">Worked situation</span><p class="body-copy">${escape(example)}</p></section>` : '';
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    *{box-sizing:border-box}body{margin:0}.slide-container{width:1280px;min-height:720px;background:#f7f8fa;color:#152238;font-family:Arial,Helvetica,sans-serif;padding:44px 62px;position:relative;overflow:hidden}.eyebrow{font-size:14px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase;color:#a56508}.title-row{display:flex;align-items:flex-start;justify-content:space-between;gap:30px}.title{max-width:950px;margin:10px 0 13px;font-size:47px;line-height:1.08;letter-spacing:-1.2px}.stage{font-size:15px;font-weight:800;color:#64748b;white-space:nowrap;margin-top:16px}.rule{height:5px;width:170px;background:linear-gradient(90deg,#ff9900 0 33%,#00875a 33% 66%,#de350b 66%);margin:0 0 18px}.lesson-grid{display:grid;grid-template-columns:1fr 1.1fr;gap:18px;align-items:stretch}.visual-panel,.cover-aid,.exercise,.safety-grid,.evidence-panel,.assessment-grid{min-height:330px;padding:22px 24px;background:#fff;border:1px solid #d8dee8}.explain-panel{min-height:330px;padding:22px 24px;background:#fff;border-top:5px solid #00875a}.label{display:block;margin-bottom:9px;color:#a56508;font-size:13px;letter-spacing:1.3px;font-weight:800;text-transform:uppercase}.visual-term{margin:-4px 0 11px;font-size:25px;font-weight:800;line-height:1.1}.body-copy{font-size:21px;line-height:1.34;margin:0}.example-label{margin-top:18px}.step-flow{display:flex;align-items:stretch;gap:7px;margin:15px 0 18px}.step-flow i{align-self:center;color:#a56508;font-style:normal;font-size:19px}.step{flex:1;min-width:0;padding:13px 8px;background:#eef3f7;text-align:center;font-size:15px;line-height:1.18}.step b{display:block;width:22px;height:22px;margin:0 auto 7px;border-radius:50%;background:#ff9900;color:#152238;line-height:22px}.visual-caption{font-size:17px;line-height:1.35;color:#475569;margin:0}.cover-layout{display:grid;grid-template-columns:1.25fr .75fr;gap:20px;align-items:center}.cover-graphic{margin:0;text-align:center}.cover-graphic img{width:300px;height:185px;object-fit:contain}.cover-graphic figcaption{margin-top:3px;color:#64748b;font-size:12px}.table-panel table,.evidence-panel table{width:100%;border-collapse:collapse;font-size:16px}.table-panel th,.table-panel td,.evidence-panel th,.evidence-panel td{padding:9px 10px;border-bottom:1px solid #d8dee8;text-align:left;vertical-align:top}.table-panel th,.evidence-panel th{background:#152238;color:#fff;font-size:13px}.compass{position:relative;width:190px;height:190px;margin:3px auto 8px;border:2px solid #d8dee8;border-radius:50%;background:#f5f8fa}.compass-core{position:absolute;inset:62px 37px;display:flex;align-items:center;justify-content:center;padding:8px;border-radius:50%;background:#ff9900;font-size:16px;font-weight:800;text-align:center}.compass-point{position:absolute;font-size:11px;font-weight:800;letter-spacing:.8px;color:#00875a}.north{top:14px;left:66px}.east{right:10px;top:87px}.south{bottom:14px;left:60px}.west{left:10px;top:87px}.action{margin-top:16px;padding:13px 18px;background:#152238;color:#fff;font-size:18px;line-height:1.3}.action strong{color:#ffbf55}.cover-aid{min-height:300px}.exercise-question{max-width:910px;font-size:26px;font-weight:800;line-height:1.25;margin:0 0 18px}.choice-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.choice-btn{min-height:76px;padding:12px;border:2px solid #cbd5e1;background:#f8fafc;color:#152238;font:700 16px/1.25 Arial,Helvetica,sans-serif;text-align:left;cursor:pointer}.choice-btn:hover,.choice-btn:focus-visible{border-color:#ff9900;outline:3px solid rgba(255,153,0,.22)}.choice-btn.is-correct{background:#e8f5ef;border-color:#00875a}.choice-btn.is-incorrect{background:#fff0ed;border-color:#de350b}.feedback{min-height:24px;margin:15px 0 8px;font-size:17px;font-weight:700}.debrief{padding:11px 14px;background:#eef3f7;font-size:16px;line-height:1.3}.safety-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.safe,.risk{padding:22px;font-size:21px;line-height:1.35}.safe{background:#e8f5ef;border-top:5px solid #00875a}.risk{background:#fff0ed;border-top:5px solid #de350b}.safe p,.risk p{margin:0}.assessment-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.assessment-grid>div{padding:19px;background:#eef3f7;border-top:5px solid #ff9900}.assessment-grid>div:nth-child(2){border-color:#00875a}.assessment-grid>div:nth-child(3){border-color:#152238}.assessment-grid b{font-size:21px;line-height:1.2}.assessment-grid p{margin:10px 0 0;font-size:17px;line-height:1.3}.footer{position:absolute;left:62px;right:62px;bottom:18px;display:flex;justify-content:space-between;color:#64748b;font-size:13px}.reveal{opacity:0;transform:translateY(12px);animation:reveal .48s ease-out both}.d1{animation-delay:.08s}.d2{animation-delay:.16s}.d3{animation-delay:.24s}@keyframes reveal{to{opacity:1;transform:none}}@media (prefers-reduced-motion:reduce){.reveal{opacity:1;transform:none;animation:none}.choice-btn{transition:none}}
  </style>
</head>
<body>
  <div class="slide-container">
    ...
    <main class="${isConcept ? 'lesson-grid' : 'special-stage'}">${aid}${explanation}</main>
  ...
  <script>document.querySelectorAll('.choice-btn').forEach(button=>button.addEventListener('click',()=>{const correct=button.dataset.correct==='true';document.querySelectorAll('.choice-btn').forEach(choice=>choice.classList.remove('is-correct','is-incorrect'));button.classList.add(correct?'is-correct':'is-incorrect');const feedback=document.querySelector('.feedback');if(feedback)feedback.textContent=correct?'Good choice. Read the debrief, then write the reason.':'Not yet. Read the debrief and choose the evidence-based action.';}));</script>
</body>
</html>`;
}
...
for (const module of modules) {
  ...
  const slides = [
    ['cover', 'Start here', module.title, 'Today’s goal', module.goal, 'You do not need Amazon experience to begin. Learn one small idea, then use it in a safe practice task.', 'Read the title and say what you want to learn today.'],
    ...module.points.map(([term, meaning, example, action]) => ['concept', 'One idea at a time', term, term, meaning, example, action]),
    ['exercise', 'Try it', 'Use a small situation', 'Decision practice', 'Choose the safest first step from the short situation.', module.scenario, 'Choose an answer, read the debrief, and write your reason.'],
    ['guardrail', 'Keep it safe', 'Know what not to change yet', 'Pause first', 'Good VA work is careful. Stop when a product or account problem needs checking before an ad change.', module.guardrail, 'Say what you would not change yet.'],
    ['evidence', 'Show your work', 'Write a simple evidence note', 'Evidence note', 'A short note makes your decision easy for a manager or another VA to understand.', module.evidence, 'Copy the evidence note into your workspace after practice.'],
    ['assessment', 'Finish the module', 'Prove one small skill', 'Assessment gate', `Complete the ${module.practice} practice at ${module.score}. Then pass the module quiz at ${module.quiz}.`, 'Do the practice first. Use your evidence note to explain one decision.', 'Complete the practice, save your note, then take the quiz.']
  ];
  for (const [index, data] of slides.entries()) {
    const [kind, eyebrow, title, term, meaning, example, action] = data;
    await writeFile(path.join(folder, `slide_${index + 1}.html`), slide(module, index + 1, kind, eyebrow, title, term, meaning, example, action));
  }
}
